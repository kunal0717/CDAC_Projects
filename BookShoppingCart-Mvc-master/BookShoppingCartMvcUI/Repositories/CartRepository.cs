    ...
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<bool> DoCheckout(CheckoutModel model)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Order status does not have Pending status");
                    throw new InvalidOperationException("Order status does not have Pending status");
                }
                ...
                foreach (var item in cartDetail)
                {
                    ...
                    if (stock == null)
                    {
                        Console.WriteLine("Stock is null for BookId: " + item.BookId);
                        throw new InvalidOperationException("Stock is null");
                    }

                    if (item.Quantity > stock.Quantity)
                    {
                        Console.WriteLine($"Only {stock.Quantity} item(s) are available in stock for BookId: " + item.BookId);
